...
export default function Chat() {
  ...
  const handleDeleteSession = async (key: string, e: React.MouseEvent) => {
    e.stopPropagation()
    if (!confirm('ConfirmerSupprimerSessions？')) return
    try {
  ...
  const handleAnnulerRename = useCallback(() => {
  ...
  }, [fetchSessions, handleAnnulerRename, renameSaving, renameValue])
    ...
    <div className="flex h-[calc(100vh-3rem)] -m-6">
      ...
      <div className="w-64 border-r border-dark-border bg-dark-sidebar flex flex-col shrink-0">
        <div className="px-4 py-3 border-b border-dark-border flex items-center justify-between">
          <h2 className="text-sm font-semibold text-dark-text">Sessions</h2>
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {activeSessionKey ? (
              ...
              <div className="flex items-center gap-2">
                {(sending || agentRunning) && (
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent-blue/10 border border-accent-blue/20 animate-pulse">
                    ...
                    <span className="text-[11px] text-accent-blue font-medium">En cours</span>
                  </div>
                )}
                <button
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              {chatLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-dark-text-secondary">
                  ...
                  <p className="text-xs mt-1 opacity-60">Téléversement dimages et fichiers</p>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <div className="flex flex-col items-start max-w-[80%]">
                          ...
                          {msg.role !== 'user' && msg.content?.trim() && (
                            ...
                            >
                              {copiedIdx === i ? <><Check size={12} /> Copié</> : <><Copy size={12} /> </>}
                            </button>
                          )}
                        </div>
                        {msg.role === 'user' && (
                        ...
                        <div className="rounded-xl px-4 py-2.5 bg-dark-card border border-dark-border">
                          {displayedText ? (
                            ...
                            <div className="flex items-center gap-2 text-sm text-dark-text-secondary">
                              ...
                              {liveToolCalls.some(t => t.status === 'running') ? 'Outils...' : '...'}
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <div className="max-w-3xl mx-auto flex items-end gap-2">
                ...
                <div className="relative flex-1">
                  ...
                  <textarea
                    ...
                    onPaste={handlePaste}
                    placeholder={pendingFiles.length > 0 ? '（）...' : 'Message，， / ...'}
                    rows={1}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-dark-text-secondary">
            ...
            <p className="text-sm mb-4">SessionsSessions</p>
            <button
              onClick={handleNewSession}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="rounded-xl bg-dark-card border border-dark-border max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-dark-border">
              <h3 className="text-base font-semibold text-dark-text"> Agent</h3>
              <button
