...
_IDENTITY_NAME_RE = re.compile(r"(?:\*\*)?\s*(?:/*TODO-fr*/|name)\s*(?:：|:)\s*(?:\*\*)?\s*(.+)", re.IGNORECASE)
...
def parse_identity(agent_id, text, meta=None):
    ...
    if not name:
        for line in lines:
            lower = line.lower()
            if "name" in lower or "/*TODO-fr*/" in line:
                value = re.sub(r"^[\-\*\s]*", "", line)
                value = re.sub(r"(\*\*)?(name|/*TODO-fr*/)(\*\*)?\s*[:：]\s*(\*\*)?", "", value, flags=re.I).strip()
    ...
    if not description:
        ...
        if marker in text:
            ...
            body_lines = [line for line in lines if not line.startswith("#") and "name" not in line.lower() and "/*TODO-fr*/" not in line]
...
NAME_RE = re.compile(r'(?:\*\*)?\s*(?:/*TODO-fr*/|name)\s*(?:：|:)\s*(?:\*\*)?\s*(.+)', re.IGNORECASE)
...
def create_agent_in_container(container_id: str, agent_id: str) -> dict:
    ...
    if result.exit_code == 0:
        raise HTTPException(status_code=409, detail=f"Agent '{agent_id}' /*TODO-fr*/")
...
(profile / 'SOUL.md').write_text(
    '# SOUL.md - {agent_id}\\n\\n'
    '/*TODO-fr*/ {agent_id}，/*TODO-fr*/ AI /*TODO-fr*/。\\n'
    '/*TODO-fr*/、/*TODO-fr*/Utilisateur/*TODO-fr*/。\\n',
    encoding='utf-8',
)
...
"""
    ...
    if result.exit_code != 0:
        ...
        raise HTTPException(status_code=500, detail=f"créer Agent /*TODO-fr*/: {output}")
...
def delete_agent_from_container(container_id: str, agent_id: str) -> dict:
    ...
    result = container.exec_run(["test", "-d", profile_dir], user="hermes")
    if result.exit_code != 0:
        raise HTTPException(status_code=404, detail=f"Agent '{agent_id}' /*TODO-fr*/")

    result = container.exec_run(["rm", "-rf", profile_dir], user="hermes")
    if result.exit_code != 0:
        raise HTTPException(status_code=500, detail="supprimer Agent /*TODO-fr*/")
