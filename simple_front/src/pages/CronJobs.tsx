          ...
          <div className="space-y-5">
            ...
            <label className="block">
              ...
              <span className="mt-1 block text-xs text-light-text-secondary">
                定时触发时，此消息将作为用户输入发送给 Agent，执行结果会写入一个可打开的会话。
              </span>
            </label>
            ...
            {scheduleType === 'every' && (
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium text-light-text-secondary">间隔分钟数</span>
                <ClearableInput
                  type="number"
                  min={1}
                  step={1}
                ...
                />
                <span className="mt-1 block text-xs text-light-text-secondary">
                  {everyPreview}。Hermes 当前按分钟调度，低于 1 分钟的间隔不会被保存。
                </span>
              </label>
            )}
    ...
    <div className="h-full overflow-y-auto bg-light-bg">
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-light-text-secondary">
              管理 Agent 的自动执行计划。任务执行后，结果会进入对应会话并触发左侧未读提示。
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex min-h-72 flex-col items-center justify-center rounded-xl border border-dashed border-light-border bg-light-card px-5 py-12 text-center">
              ...
              <div className="mt-1 max-w-sm text-sm leading-6 text-light-text-secondary">
                创建固定间隔、Cron 或单次执行的任务，让 Agent 在指定时间自动处理消息。
              </div>
              <button
                type="button"
                onClick={() => setCreateOpen(true)}
                ...
                {jobs.map(job => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {job.session_key && (
                        <IconButton label="打开结果会话" onClick={() => openSession(job.session_key)}>
      ...
      <CreateCronPanel
        ...
        onCreated={job => {
          ...
          toast.success(`已创建“${getJobTitle(job)}”，执行结果会进入左侧会话`)
          setCreateOpen(false)
        }}
      />
    </div>
