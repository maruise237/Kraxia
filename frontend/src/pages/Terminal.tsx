...
export default function TerminalPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-7.5rem)] flex-col">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-dark-text">Terminal temps réel</h1>
        <p className="mt-1 text-sm text-dark-text-secondary">（xterm.js + PTY），、Tab 、TUI （vi/top/htop）。。</p>
      </div>
      ...
      <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-dark-border bg-dark-card">
        <div className="px-5 py-3 border-b border-dark-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-dark-text">Terminal temps réel（xterm.js + PTY）</h2>
          </div>
          <span className={`text-xs ${termConnected ? 'text-accent-green' : 'text-dark-text-secondary'}`}>
            {termConnected ? 'Connecter' : 'Connecter'}
          </span>
        </div>
