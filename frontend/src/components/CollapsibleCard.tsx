...
function toolDisplayName(name: string): string {
  switch (name) {
    case 'skill_view': return 'Chargement de compétence'
    case 'terminal': return 'Exécution de commande'
    case 'run_command': return 'Exécution de commande'
    case 'view_file': return 'Fichier'
    case 'write_file': return 'Fichier'
    case 'edit_file': return 'ModifierFichier'
    case 'skills_list': return 'Liste des compétences'
    case 'web_search': return 'Recherche web'
    case 'browser_navigate': return 'Navigation web'
    default: return name
  }
}
...
function toolResultSummary(content: string): { status: 'success' | 'error' | 'neutral'; summary: string } {
  try {
    ...
    if (parsed.success === true && parsed.name) {
      return { status: 'success', summary: `Chargé :  ${parsed.name}` }
    }
    if (parsed.success === false) {
      return { status: 'error', summary: parsed.error || 'Échec du chargement' }
    }
    // terminal result
    if ('exit_code' in parsed) {
      if (parsed.status === 'blocked') {
        return { status: 'error', summary: 'Rejeter' }
      }
    ...
    // view_file result
    if (parsed.content) {
      return { status: 'success', summary: 'Fichier' }
    }
    ...
    <div className="my-1.5 max-w-3xl space-y-1">
      {tools.map(tc => (
        ...
        >
          ...
          {tc.status === 'running' && (
            <span className="ml-auto text-[10px] text-accent-blue/70">...</span>
          )}
